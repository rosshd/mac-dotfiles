...
This information can be used for secure-by-default implementation or an explicitly requested security review.

Do not turn ordinary code review, debugging, or implementation into a full security audit.

If ordinary work exposes an unmistakable critical vulnerability, warn the user concisely without expanding into the full report workflow.
...
2. During ordinary work, only an unmistakable critical issue warrants a concise warning; this does not activate the full review workflow.
...
When producing fixes, focus on one coherent finding at a time and add comments only when the security invariant is not clear from the code.
